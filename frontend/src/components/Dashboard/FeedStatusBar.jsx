 ...
 *
 * Shows real Pyth prices when available.
 * Shows a "SIMULATED" badge if falling back to demo prices.
 */
...
export function FeedStatusBar({ isDemoMode = false }) {
  const { feedStatuses, isLoadingFeeds, isUsingRealPrices } = useFeeds(isDemoMode);
  ...
  return (
    <div style={containerStyle}>
      ...
      {isUsingRealPrices ? (
        <span style={realBadgeStyle}>● LIVE</span>
      ) : (
        <span style={simBadgeStyle}>○ SIMULATED</span>
      )}
...
function FeedPill({ feedStatus }) {
  ...
  const symbol    = feedStatus.asset_symbol
    .replace("Crypto.", "")
    .replace("/USD", "");
    ...
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "1px", flexShrink: 0 }}>
      <div style={{ display: "flex", alignItems: "center", gap: "4px" }}>
        <div style={{
          ...
          background: feedStatus.is_feed_fresh ? "#22c55e" : "#ef4444",
        }} />
      ...
      <span style={{ color: "#94a3b8", fontSize: "11px", fontFamily: "monospace" }}>
        ${feedStatus.normalized_price.toLocaleString(undefined, {
